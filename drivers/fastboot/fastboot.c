...
#define EANAB_MBR_OFFSET	0
#define EANAB_MBR_SIZE	    	0x400 /*1024*/
#define EANAB_BOOTLOADER_OFFSET 0 
#define EANAB_BOOTLOADER_SIZE	0x5E000 /*376*1024*/
#define EANAB_KERNEL_OFFSET	0x41000
#define EANAB_KERNEL_SIZE	0xE00000 /*14*1024*1024*/
#define EANAB_SYSTEM_OFFSET	0x2000000
#define EANAB_SYSTEM_SIZE	0x1C200000 /*450*1024*1024*/
...
#ifdef CONFIG_CMD_IDME
static u8 wstrSerial[2*(SERIAL_NUM_LEN)+2];
#else
...
#ifndef CONFIG_FASTBOOT_STORAGE_VENDOR_PTABLE
...
#ifdef CONFIG_CMD_IDME
	fastboot_interface->serial_no = serial_number;
#else
...
static void __def_fastboot_load_partitions(void)
{
#ifndef CONFIG_FASTBOOT_STORAGE_VENDOR_PTABLE
...
void board_fastboot_load_partitions(void) \
	__attribute__((weak, alias("__def_fastboot_load_partitions")));
...
void fastboot_setup(void)
{
	...
	/* execute board relevant partition load */
	board_fastboot_load_partitions();
}
...
static int _fastboot_parts_load_from_ptable(void)
{
	...
	ptable[PTN_BOOTLOADER_INDEX].start =
				EANAB_BOOTLOADER_OFFSET / dev_desc->blksz;
	ptable[PTN_BOOTLOADER_INDEX].length =
				 EANAB_BOOTLOADER_SIZE / dev_desc->blksz;
	ptable[PTN_BOOTLOADER_INDEX].partition_id = boot_partition;

	printf("ptable for bootloader: start=%d,size=%d,partitionid=%d\n",ptable[PTN_BOOTLOADER_INDEX].start,ptable[PTN_BOOTLOADER_INDEX].length,ptable[PTN_BOOTLOADER_INDEX].partition_id);
	_fastboot_parts_add_ptable_entry(PTN_KERNEL_INDEX,
				   CONFIG_ANDROID_BOOT_PARTITION_MMC,
				   user_partition, "kernel", dev_desc, ptable);
...
#endif /* CONFIG_FASTBOOT_STORAGE_VENDOR_PTABLE */
...
/*
 * Initialize partition table on storage device.
 * !!!Currently supported GPT partition layout on MMC device ONLY!!!
 */
unsigned int fastboot_flash_init_partitions(void)
{
	...
	int			i;
	int			count = 0;
	disk_partition_t	gpt_table[MAX_PTN];
	int			mmc_no = 0;
	struct			mmc *mmc;
	block_dev_desc_t	*dev_desc;

	if (fastboot_devinfo.type != DEV_MMC) {
		printf("Supported MMC only!\n");
		return -1;
	}
	...
	for (i = 0; i < pcount; i++) {
		/* all user partitions only */
		if (ptable[i].partition_id == FASTBOOT_MMC_BOOT_PARTITION_ID)
			continue;

		strcpy((char *)gpt_table[count].name, ptable[i].name);
		gpt_table[count].start = ptable[i].start;
		gpt_table[count].size = ptable[i].length;

		printf("%d: %-20s %08lx    %08lx\n", count,
				gpt_table[count].name,
				gpt_table[count].start,
				gpt_table[count].size);
		/* next */
		count++;
	}
	...
	/* update GPT */
	if (count) {
		const char *str_disk_guid = "55465300-0000-11AA-AA11-00306543ECAC";
		ret = gpt_restore(&mmc->block_dev, (char *)str_disk_guid, gpt_table, count);
	}
#endif
	return ret;
}
