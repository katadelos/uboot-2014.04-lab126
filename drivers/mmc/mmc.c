...
static struct mmc g_mmc[CONFIG_SYS_FSL_USDHC_NUM];
int g_cur_mmc = 0;
#endif
...
static int mmc_startup(struct mmc *mmc)
{
	...
	printf("FRED::: EXTCSD177=%x, 179=%x",ext_csd[EXT_CSD_BOOT_BUS_WIDTH],ext_csd[EXT_CSD_PART_CONF]);
...
struct mmc *mmc_create(const struct mmc_config *cfg, void *priv)
{
	...
	if (cfg == NULL || cfg->ops == NULL || cfg->ops->send_cmd == NULL ||
		...
		return NULL;
#ifdef CONFIG_CMD_IDME
	if ( g_cur_mmc >= CONFIG_SYS_FSL_USDHC_NUM ) 
		return NULL;
        mmc = &(g_mmc[g_cur_mmc++]);
