...
unsigned int get_mtest_end_addr(void)
{
	return gd->start_addr_sp - SZ_256K;	// subtract a little for stack
}
	...
	while(1)
		...
		board = get_board_type();
	      		...
	      		switch (board->mem_type) 
			{
				...
				{
                        		//lpddr2_init(board);//TODO: Need to add this function back from wario platform. just in case Lab126 support diff size/settings of DDR
		    		} 
				else 
				{
...
int board_mmc_init(bd_t *bis)
{
	...
	for (i = 0; i < CONFIG_SYS_FSL_USDHC_NUM; i++) {
		...
		case 1:
			...
			/* Initialize it to use 1.8V for signal voltage */
			usdhc_cfg[1].signal_voltage = ESDHC_VOLTAGE_180;
			break;
		case 2:
			setup_iomux_usdhc3();
...
#define NEW_BOOTCMD_LENGTH 21
	...
	if(rebootmode == ANDROID_BOOT_FASTBOOT) {
	  ...
	  else if (!strncmp(boot_mode, "reset", strlen("reset"))) {
	    printf ("BOOTMODE OVERRIDE: RESET\n");
	    strcpy(boot_cmd, "reset");
        } else {
...
void board_entry_idle_mode(void)
{
	disable_pm_epd();
	setup_iomux_idle();
	gpio_direction_output( IMX_GPIO_NR(5,10), 0);
