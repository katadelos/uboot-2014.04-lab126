...
static ulong mem_test_quick(vu_long *buf, ulong start_addr, ulong end_addr,
			    vu_long* ppattern, int iteration)
{
	vu_long *end;
	vu_long *addr;
	ulong errs = 0;
	...
	if (iteration & 1)
		incr = -incr;
	if (iteration > 0) {
		if (*ppattern & 0x80000000)
			*ppattern = -*ppattern;	/* complement & increment */
		else
			*ppattern = ~*ppattern;
	}
	...
	for (addr = buf, val = *ppattern; addr < end; addr++) {
		WATCHDOG_RESET();
		*addr = val;
		val += incr;
	}
			...
			char * const argv[])
{
	...
	if (argc > 3)
		pattern = (vu_long)simple_strtoul(argv[3], NULL, 16);
	else
		pattern = 0;
