...
#ifdef CONFIG_CMD_IDME
#define IDME_MAX 30
int fdt_idme(void *fdt,int nodeoffset, const char *name, int size)
{
	int err;
	unsigned char idme_buf[IDME_MAX];
       if (idme_get_var(name, (char *) idme_buf, size+1))
        {
                /* not found: clean up garbage characters. */
                memset(idme_buf, 0, IDME_MAX);
        }

	err = fdt_setprop(fdt, nodeoffset,name,idme_buf , size+2);
	if (err < 0){
		printf("WARNING: could not set idme path %s %s.\n",name,fdt_strerror(err));
		return err;
	}
	return 0;
}
...
int fdt_chosen(void *fdt, int force)
{
	...
	for (i = 0; i < CONFIG_NUM_NV_VARS; i++) {
		err = fdt_idme(fdt,nodeoffset,  nvram_info[i].name, nvram_info[i].size);
	}
