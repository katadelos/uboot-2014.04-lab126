...
static int idme_is_hexval(char c)
{
	if ((c >= '0' && c <= '9') 
		|| (c >= 'a' && c <= 'f')
		|| (c >= 'A' && c <= 'F'))
...
static int idme_is_validate_mac(const char *value)
{
	int i;
	for (i = 0; i < 12; i++) {
		if(!idme_is_hexval(value[i]))
			return 0;
	}
	return 1;
}
...
int idme_update_var(const char *name, const char *value) 
{
	...
	if((strncmp(nv->name, "mac", 3)==0) || (strncmp(nv->name, "btmac", 5)==0)) {
		if(!idme_is_validate_mac(value)) {
			printf("Error! Wrong MAC string: %s::%s\n", name,value);
			return -1;
		}
	}
